...
#     for snr in snr_list:
#         # print(snr)
#         comm_sys = Transmitter_Receiver(3000, modulation)
#         comm_sys.send_n_receive(snr)
#         comm_sys.decode()
...
class LLR_net():
    ...
    '''
        ...
        print(f"X: {self.X.shape}"); print(f"y: {self.y.shape}")
...
#   axes[n].semilogy(snr_list, th_error[n], label = 'Theoretical BER')
#   axes[n].set_title(modulation)
...
# train_snr = 25
# for n, modulation in enumerate(modulation_list):
#   a = LLR_net(modulation, 20000, 10000, train_snr = train_snr, activation = 'relu',neuron_size=32)
#   a.epoch_size = 50
#   a.train()
#   for snr in snr_list:
#     # a = LLR_net(modulation, 10000, 20000, train_snr = snr, test_snr = snr, activation = 'relu')
...
# for n, modulation in enumerate(modulation_list):
#   for snr in snr_list:
#     a = LLR_net(modulation, 5000, 10000, train_snr = snr, test_snr = snr, activation = 'tanh')
#     a.train()
#     a.test()
#     llr_conventional[n].append(a.b_error)
#     conventional_error[n].append(a.conventional_error)
...
# SNR_Range = [-10,31] # 0,31
# spacing = 1
# test_snr_list = np.linspace(SNR_Range[0],SNR_Range[1],int(np.abs(SNR_Range[0]-SNR_Range[1])*1/spacing)+1)
#
#
# # In[78]:
#
...
# spacing = 1
# test_snr_list = np.linspace(SNR_Range[0],SNR_Range[1],int(np.abs(SNR_Range[0]-SNR_Range[1])*1/spacing)+1)
# train_snr_list = np.array([-10, -5, 0, 5, 10, 15, 20, 25, 30])
#
# modulation_list = ['BPSK', '4QAM', '16QAM', '64QAM']
#
# fig, ((ax1, ax2, ax3, ax4),
...
#       (ax51, ax52, ax53, ax54),
#       (ax61, ax62, ax63, ax64)
#       ) = plt.subplots(9,4, figsize = (100,25))
#
# axes = [[ax1, ax2, ax3, ax4],
#         [ax6, ax7, ax8, ax9],
#         [ax11, ax12, ax13, ax14],
...
#         if n == 0:
#           a = LLR_net(modulation, 5000, 10000, train_snr = train_snr, test_snr = test_snr, activation = 'tanh')
#           a.train()
#         a.test_snr = test_snr
#         a.test()
...
#
#     axes[j][i].semilogy(test_snr_list, llr_net_error, linewidth = 5, label = 'Neural Net BER')
#     axes[j][i].semilogy(test_snr_list, conventional_error, label = 'Conventional BER')
#     axes[j][i].semilogy(test_snr_list, th_error, label = 'Theoretical BER')
#     if i == 0:
#       axes[j][i].set_title('Train SNR: ' + str(train_snr))
#     if j == 0:
#       axes[j][i].set(ylabel = modulation + 'Bit Error Rate')
#       axes[j][i].legend()
#     if i == 9:
#       axes[j][i].set(xlabel = 'snr (dB)')
#
#
#
# # In[80]:
...
#
# for n, modulation in enumerate(modulation_list):
#   a = LLR_net(modulation, 5000, 10000, train_snr = train_snr, test_snr = -10, activation = 'tanh',
...
#
# for n, modulation in enumerate(modulation_list):
#   a = LLR_net(modulation, 5000, 10000, train_snr = train_snr, test_snr = snr, activation = 'tanh', epoch_size= 300)
#   a.train()
#   for snr in snr_list:
#     a.test_snr = snr
#     a.test()
...
#
#
# snr_list = np.linspace(-10, 40, 20)
# layer_num_list = [1, 2, 3, 4]
# modulation = '64QAM'
# fig, (ax1, ax2, ax3, ax4) = plt.subplots(1, 4, figsize = (25, 5))
# axes = [ax1, ax2, ax3, ax4]
# for n, layer_num in enumerate(layer_num_list):
#   a = LLR_net(modulation, 5000, 10000, train_snr = 30, test_snr = 5, activation = 'tanh',
